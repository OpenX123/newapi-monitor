...
- **`prompt_tokens` 已包含缓存读取 token**。按 OpenAI usage 口径，「总 Token」= `prompt_tokens + completion_tokens`；`cache_tokens` 仅用于把输入拆成新输入和缓存输入，不再重复计入总量。缓存部分通过 `CACHE_TOKENS_EXPR` 从 `other` 里单独提取。
