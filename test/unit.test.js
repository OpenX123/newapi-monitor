...
function testFormatters() {
  ...
  const cell = tokenUsageCell({ total_tokens: 1300, prompt_tokens: 900, completion_tokens: 100, cache_tokens: 600 });
  check('有缓存时拆出「入 + 缓存」', cell.includes('缓存'), '');
  check('缓存按半价重复计入总量', cell.includes('1,300'), '');
...
function testUsageSemantics() {
  ...
  check('token 用量只对消费日志求和且缓存按半价计入', /SUM\(COALESCE\(prompt_tokens, 0\) \+ COALESCE\(completion_tokens, 0\) \+ \(\$\{CACHE_TOKENS_EXPR\} \* 0\.5\)\) FILTER \(WHERE type = 2\)/.test(src));
