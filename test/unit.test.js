...
function testFormatters() {
  ...
  check('缓存不重复计入总量', cell.includes('300') || cell.includes('入 300'), '');
...
function testUsageSemantics() {
  ...
  check('token 用量只对消费日志求和', /SUM\(COALESCE\(prompt_tokens, 0\) \+ COALESCE\(completion_tokens, 0\)\) FILTER \(WHERE type = 2\)/.test(src));
